...
Latest images can be found at:
- [GitHub Container Registry (ghcr.io/techtricks-ai/docker-external-dns)](https://github.com/techtricks-ai/docker-external-dns/pkgs/container/docker-external-dns).
- [Docker Hub (techtricks/docker-external-dns)](https://hub.docker.com/r/techtricks/docker-external-dns)
...
### 4. Deploying Multiple Instances

You can run multiple instances of `docker-external-dns` side-by-side by leveraging the `IDENTIFIER` and `DOMAIN_FILTER` variables to keep your environments completely isolated.
**Common Use Case:** Running one instance dedicated to standard Docker container labels as the source, and another instance dedicated to Traefik routers, with both pushing to the same DNS provider.

```yaml
services:
  # Instance 1: Synced with Docker labels
  external-dns-docker:
    image: ghcr.io/techtricks-ai/docker-external-dns:latest
    environment:
      - IDENTIFIER=docker-source
      - SOURCE=docker

  # Instance 2: Synced with Traefik routers
  external-dns-traefik:
    image: ghcr.io/techtricks-ai/docker-external-dns:latest
    environment:
      - IDENTIFIER=traefik-source
      - SOURCE=traefik
```
> [!NOTE]
> The `IDENTIFIER` variable defaults to `docker-external-dns`. When running multiple instances, you must set a unique identifier for each deployment to ensure proper tracking and cleanup of stale records.

### 5. Advanced: Multi-Host Synchronization
...
This project is configured with a GitHub Actions pipeline that builds, tests, and automatically tags and releases new versions.
